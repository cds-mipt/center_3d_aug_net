...
CPFOLDER = '/disks/hdd/pku/article/huber'
...
def draw_line(image, points):
    color1 = (255, 0, 0)
    color2 = (0, 255, 0)
    cv2.line(image, tuple(points[0][:2]), tuple(points[3][:2]), color1, 8)
    cv2.line(image, tuple(points[0][:2]), tuple(points[1][:2]), color1, 8)
    cv2.line(image, tuple(points[1][:2]), tuple(points[2][:2]), color1, 8)
    cv2.line(image, tuple(points[2][:2]), tuple(points[3][:2]), color2, 8)
    return image
...
def criterion(prediction, mask, regr, size_average=True):
    ...
    pos_regr = regr[:, [3, 4, 6]]
    rot_regr = regr[:, [0, 1, 2, 5]]
    
    pos_pred = pred_regr[:, [3, 4, 6]]
    rot_pred = pred_regr[:, [0, 1, 2, 5]]
    
    
    pos_loss = huber_loss(pos_pred, pos_regr, beta=2.8)
    
    
    rot_loss = (torch.abs(rot_pred - rot_regr).sum(1) * mask).sum(1).sum(1) / (1e-5 + mask.sum(1).sum(1))
    rot_loss = rot_loss.mean(0)

    
    loss = mask_loss + pos_loss + rot_loss
    if not size_average:
        loss *= prediction.shape[0]
    return loss, mask_loss, pos_loss, rot_loss
...
def calk_map(filename):
    ...
    train_df_ = pd.read_csv('/datasets/pku-autonomous-driving/train.csv')
...
def train(epoch, history=None):
    ...
    for batch_idx, (img_batch, mask_batch, regr_batch) in enumerate(t):
        ...
        loss, mask_loss, pos_loss, rot_loss = criterion(output, mask_batch, regr_batch)
        
        t.set_description(f'train_loss (l={loss:.3f})(m={mask_loss:.2f}) (p={pos_loss:.4f} (r={rot_loss:.4f}')
    ...
    writer.add_scalar('train/rot_loss', train_rot_loss, epoch)
    
    print('Train Epoch: {} \tLR: {:.6f}\tLoss: {:.6f}\tMaskLoss: {:.6f}\tPosLoss: {:.6f}\tRotLoss: {:.6f}'.format(
        epoch,
...
def evaluate(epoch, history=None):
    ...
    with torch.no_grad():
        for img_batch, mask_batch, regr_batch in tqdm(dev_loader):
            ...
            loss, mask_loss, pos_loss, rot_loss = criterion(output, mask_batch, regr_batch, size_average=False)
            valid_loss += loss.data
            valid_mask_loss += mask_loss.data
            valid_pos_loss += pos_loss.data
            valid_rot_loss += rot_loss.data
    ...
    valid_pos_loss /= len(dev_loader.dataset)
    valid_rot_loss /= len(dev_loader.dataset)
    ...
    writer.add_scalar('dev/pos_loss', valid_pos_loss, epoch)
    writer.add_scalar('dev/rot_loss', valid_rot_loss, epoch)
    ...
    if history is not None:
        ...
        history.loc[epoch, 'pos_loss'] = valid_pos_loss.cpu().numpy()
        history.loc[epoch, 'rot_loss'] = valid_rot_loss.cpu().numpy()
    
    print('Dev: mAP: {:.6f}\tLoss: {:.6f}\tMaskLoss: {:.6f}\tPosLoss: {:.6f}\tRotLoss: {:.6f}'.format(
        map,
        valid_loss, 
        valid_mask_loss, 
        valid_pos_loss,
...
model.load_state_dict(torch.load(os.path.join(CPFOLDER, 'epoch_55.pth'))['model'])
