...
CPFOLDER = '/disks/hdd/pku/article/geom_aug_mid'
...
class CarDataset(Dataset):
    ...
    def get_3d_transform_matrix(self):
        A = np.eye(3)
        if np.random.uniform() < 0.5:
            alpha = np.pi * np.random.normal(scale=2) / 180
        ...
        if np.random.uniform() < 0.5:
            H, W = 2710, 3384
            scale = np.random.uniform(0.90, 1.0)
...
model.load_state_dict(torch.load(os.path.join(CPFOLDER, 'epoch_56.pth'))['model'])
