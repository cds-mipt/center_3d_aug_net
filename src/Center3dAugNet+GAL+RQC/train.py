...
CPFOLDER = '/disks/hdd/pku/article/quaternion_cos'
...
def matrix_to_quaternion(Rot):
    ...
    q = rotation.as_quat()
    e = np.array([0, 0, 0, 1], dtype=np.float32)
    q_e = q.dot(e)
    return q * q_e / np.abs(q_e)
...
class CentResnet(nn.Module):
    ...
    def forward(self, x):
        ...
        x[:, [1, 2, 3, 6]] = x[:, [1, 2, 3, 6]] / torch.norm(x[:, [1, 2, 3, 6]], dim=1)
...
def criterion(prediction, mask, regr, size_average=True):
    ...
    pos_loss = (torch.abs(pos_pred - pos_regr).sum(1) * mask).sum(1).sum(1) / (1e-5 + mask.sum(1).sum(1))
    pos_loss = pos_loss.mean(0)
    
    
    rot_loss = ((1 - (rot_regr * rot_pred).sum(1)) * mask).sum(1).sum(1) / (1e-5 + mask.sum(1).sum(1))
...
model.load_state_dict(torch.load(os.path.join(CPFOLDER, 'epoch_88.pth'))['model'])
